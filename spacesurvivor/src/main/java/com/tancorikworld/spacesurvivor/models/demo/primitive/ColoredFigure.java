...
public class ColoredFigure implements IPrimitiveFigure {
    ...
    public ColoredFigure(int radius, int vertexCount, @NonNull SimpleColor color, int program) {
        this(vertexCount, color, program);
        float[] vertices = createVertexArray(radius, vertexCount);
        mVertexData.put(vertices);
        mVertexData.position(0);
    }
    ...
    /**
     * Конструктор для создания звезды центром и определнным количеством вершин звезды
     *
     * @param radius         наружний радиус звезды
     * @param innerRadius    внутренний радиус звезды
     * @param vertexCount    количество вершин звезды
     * @param color          цвет фигуры
     * @param programCreator создатель выполнямой программы OpenGl
     */
    public ColoredFigure(int radius, int innerRadius, int vertexCount, @NonNull SimpleColor color, @NonNull OpenGlProgramCreator programCreator) {
        this(vertexCount * 2 + 4, color, programCreator);
        float[] vertices = createStarsVertexArray(radius, innerRadius, vertexCount);
        mVertexData.put(vertices);
        mVertexData.position(0);
    }
    ...
    public ColoredFigure(float wight, float height, @NonNull SimpleColor color, int program) {
        this(4, color, program);
        float[] vertices = createVertexArray(wight, height);
        mVertexData.put(vertices);
        mVertexData.position(0);
    }
    ...
    public ColoredFigure(float[] vertex, @NonNull SimpleColor color, int program) {
        this(vertex.length / 2, color, program);
        mVertexData.put(vertex);
        mVertexData.position(0);
    }
    ...
    private ColoredFigure(int vertexCount, @NonNull SimpleColor color, int program) {
        mVertexCount = vertexCount;
        mProgram = program;
        mColor = new float[] {color.getRed(), color.getGreen(), color.getBlue(), 1.0f};
        mVertexData = ByteBuffer.allocateDirect(vertexCount * 2 * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
    ...
    private float[] createStarsVertexArray(int radius, int innerRadius, int starsVertex) {
        int countVertex = starsVertex * 2;
        int specificRadius;
        float [] result = new float[countVertex * 2 + 4]; // 2 - две координаты
        result[0] = 0;
        result[1] = 0;
        for (int i = 0; i < countVertex; i++) {
            double angle =  2 * Math.PI/countVertex * i;
            specificRadius = i/2f == i/2 ? radius : innerRadius;
            double x = specificRadius * Math.cos(angle);
            int sign = angle >= Math.PI ? -1 : 1;
            double y = sign * Math.sqrt(specificRadius * specificRadius - x * x);
